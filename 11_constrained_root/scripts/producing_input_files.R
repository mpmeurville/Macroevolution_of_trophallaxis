...
### for the late ponerines

library(ape)

tree_47 = read.tree("D:/re-do_V2/00_dataset_production/output/tree_417_norm_validNames.tre")

### Prune tree to subtree from node 766

subtree = extract.clade(phy = tree_47, node = 766)
length(subtree$tip.label)

write.tree(subtree, "D:/re-do_V2/11_constrained_root/input/subtree_47.tre")

### Keep only species in subtree in sMap input files

files <- list.files("d:/re-do_V2/04_Dtest_417sp_all_traits/input", pattern="*.txt", full.names=TRUE)


  temp = read.csv(files[9], sep = "\t")
  temp = temp[temp$X417 %in% subtree$tip.label,] 
  colnames(temp) = c("47", "1")
  
  write.table(temp, "D:/re-do_V2/11_constrained_root/input/troph_47.txt", quote = F, row.names = F)
